## Pandolfi et al., 2025 Experimental assessment of the effects of sustainable landscaping practices on beetles in a new residential land development ##  
...
BD_GC1 <- glmmTMB(SR ~ BD + fR  + fGWF + BGT + VWC + I(VWC^2) + (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge, family=poisson) # Ground cover 1: GWF + BGT + VWC 
...
#Plot for full model 
coefplot.gllvm(gllvm09, y.label = TRUE, mar = c(4,15, 1, 1), cex.ylab = 0.8) 
...
rownames(fourth)  # Check trait labels
colnames(fourth)  # Check environmental variable labels

colnames(fourth) <- c(
  "Diet: COP",       # Coprophagy
  "Diet: COP+DET",   # Coprophagy + Detritivory
  "Diet: DET",       # Detritivory
  "Diet: MYC",       # Mycophagy
  "Diet: MYC+DET",   # Mycophagy + Detritivory
  "Diet: PHY",       # Phytophagy
  "Diet: POL",       # Polyphagy
  "Diet: PRD",       # Predatory
  "Wings: DIM",      # Dimorphic
  "Wings: MAC",      # Macropterous
  "Body Size",       
  "Larval Substrate: DPL+LPL",   # Dead Plant + Live Plant
  "Larval Substrate: DNG",     # Dung
  "Larval Substrate: FRT",     # Fruit
  "Larval Substrate: LPL",      # Live Plant
  "Larval Substrate: SOI",     # Soil
  "Larval Substrate: SOI+DPL",  # Soil + Dead Plant
  "Larval Substrate: SOI+DNG", # Soil + Dung
  "Larval Substrate: SOI+LPL"   # Soil + Live Plant
)
...
plot.5th <- levelplot(
  ...
  ylab = "Species Traits", 
  col.regions = colorRampPalette(c("steelblue", "white", "salmon"))(100), 
  cex.lab = 1.3, 
  at = seq(-a, b, length = 100), 
...
df1 <- data.frame(SR = Sunbridge$SR, FRic = Sunbridge$FRic)
df2 <- data.frame(SR = Sunbridge$SR, FEve = Sunbridge$FEve)
df3 <- data.frame(SR = Sunbridge$SR, FDiv = Sunbridge$FDiv)
