...
library(glmmTMB) # to run glmm models 
...
null<- glmmTMB(SR ~ 1, data = Sunbridge, family=poisson) 

OM_veg <- glmmTMB(SR ~ OM + fR + fSHR + fGWF + fUWF + fTRE + fGRA +(1|fSEASON) + (1|fBLOCK/fPLOT/fTRAP), data=Sunbridge, family=poisson) # only vegetation 

OM_GC1 <- glmmTMB(SR ~ OM + fR  + fGWF + BGT + VWC + I(VWC^2) + (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge, family=poisson) #groundcover: wildflowers, BGT, and VWC 

OM_GC2 <- glmmTMB(SR ~ OM + fR  +  fMUL + BGT + VWC + I(VWC^2) + (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge, family=poisson) #groundcover: MUL, BGT, and VWC 

OM_GC3 <- glmmTMB(SR ~ OM + fR  + fGWF + fMUL + BGT + VWC + I(VWC^2) +  (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge, family=poisson) #groundcover: wildflowers + MUL, BGT, and VWC 
...
OM_GC_soil <- glmmTMB(SR ~ OM + fR  + fGWF + fMUL + BGT + VWC + I(VWC^2) + BD + TCTN + pH + (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge, family=poisson) #groundcover, physical and chemical properties 
...
BD_veg <- glmmTMB(SR ~ BD + fR  + fSHR + fGWF + fUWF + fTRE + fGRA +  (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge, family=poisson) #only vegetation 

BD_GC1 <- glmmTMB(SR ~ sBD + fR  + fGWF + sBGT + sVWC + I(sVWC^2) + (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge, family=poisson) #groundcover: wildflowers, BGT, and VWC 

BD_GC2 <- glmmTMB(SR ~ BD + fR  +  fMUL + BGT + VWC + I(VWC^2) + (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge, family=poisson) #groundcover: MUL, BGT, and VWC 

BD_GC3 <- glmmTMB(SR ~ BD + fR  + fGWF + fMUL + BGT + VWC + I(VWC^2) +  (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge, family=poisson) #groundcover: wildflowers + MUL, BGT, and VWC 
...
BD_GC_soil <- glmmTMB(SR ~ BD + fR  + fGWF + fMUL + BGT + VWC + I(VWC^2)  + TCTN + pH + (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge, family=poisson) # groundcover, physical and chemical properties 
...
OM_veg <- glmmTMB(logCOUNT ~ OM + fR  + fSHR + fGWF + fUWF + fTRE + fGRA +  (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge) #only vegetation 

OM_GC1 <- glmmTMB(logCOUNT ~ OM + fR  + fGWF + BGT + VWC + I(VWC^2) + (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge) #groundcover: wildflowers, BGT, and VWC 

OM_GC2 <- glmmTMB(logCOUNT ~ OM + fR  +  fMUL + BGT + VWC + I(VWC^2) + (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge) #groundcover: MUL, BGT, and VWC 

OM_GC3 <- glmmTMB(logCOUNT ~ OM + fR  + fGWF + fMUL + BGT + VWC + I(VWC^2) +  (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge) #groundcover: wildflowers + MUL, BGT, and VWC 

OM_phys <- glmmTMB(logCOUNT ~ OM + fR + BD  + (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge) #physical soil properties 

OM_chem <- glmmTMB(logCOUNT ~ OM + fR + TCTN + pH + (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge) #chemical soil properties: TC:TN, pH 

OM_GC_soil <- glmmTMB(logCOUNT ~ OM + fR  + fGWF + fMUL + BGT + VWC + I(VWC^2) + BD + TCTN + pH + (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge) # groundcover, physical and chemical properties 

OM_weather <- glmmTMB(logCOUNT ~ OM + fR  + PRCP + TMIN + TMAX + (1 | fSEASON) + (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge) #-weather 

OM_full <- glmmTMB(logCOUNT ~ OM + fR   + fSHR + fGWF + fUWF + fTRE + fGRA + fMUL + BGT + VWC + I(VWC^2) + PRCP + TAVG + BD + TCTN + pH + (1 | fSEASON) + (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge) #full model 

BD_veg <- glmmTMB(logCOUNT ~ BD + fR  + fSHR + fGWF + fUWF + fTRE + fGRA +  (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge) #only vegetation 

BD_GC1 <- glmmTMB(logCOUNT ~ BD + fR  + fGWF + BGT + VWC + I(VWC^2) + (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge) #groundcover: wildflowers, BGT, and VWC 

BD_GC2 <- glmmTMB(logCOUNT ~ BD + fR  +  fMUL + BGT + VWC + I(VWC^2) + (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge) #groundcover: MUL, BGT, and VWC 

BD_GC3 <- glmmTMB(logCOUNT ~ BD + fR  + fGWF + fMUL + BGT + VWC + I(VWC^2) +  (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge) #groundcover: wildflowers + MUL, BGT, and VWC 

BD_chem <- glmmTMB(logCOUNT ~ BD + fR + TCTN + pH + OM + (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge) #chemical soil properties: TC:TN, pH, OM 

BD_GC_soil <- glmmTMB(logCOUNT ~ BD + fR  + fGWF + fMUL + BGT + VWC + I(VWC^2)  + TCTN + pH + (1 | fSEASON)+ (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge) #groundcover, physical and chemical properties 

BD_weather <- glmmTMB(logCOUNT ~ BD + fR  + PRCP + TMIN + TMAX + (1 | fSEASON) + (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge) #weather 

BD_full <- glmmTMB(logCOUNT ~ BD + fR   + fSHR + fGWF + fUWF + fTRE + fGRA + fMUL + BGT + VWC + I(VWC^2) + PRCP + TAVG + TCTN + pH + (1 | fSEASON) + (1 |fBLOCK/fPLOT/fTRAP), data=Sunbridge) #full model 
...
# Plot for VWC (with both linear and quadratic regression)
...
# Followed steps from Jenni Niku 2024 gllvm 1.4.7
...
# Transform ground cover classes in numerical variables and factors for categorical variables
...
# Calculate pseudo-R² (relative fit measure, not classical R²) 

pseudoR2_full <- 1 - (gllvm09$logL / gllvm_null$logL) 
pseudoR2_full 
...
# Plots for functional traits abundances

# Add Site column 'from TRAP1 to TRAP309' in sp.csv and add 'Species' as column name in traits.csv
...
# ex1 can be dowloaded as csv file 
...
# Model selection on FRic, FEve and FDiv
...
# Plots of Species Richness (SR) and Functional Diversity metrics
